...
<head>
    ...
    <style>
        ...
        * {{
        ...
        /* Detailed Explanation Bottom Card */
...
{clean_mermaid_code}
        ...
        <!-- Detailed Explanation Bottom Card (Underneath Graph Canvas) -->
        <div class="detail-explanation-card">
            <h2>📘 Panduan Detail Alur Eksekusi Multi-Agent Architecture</h2>
            <p class="subtitle-text">Penjelasan lengkap mengenai siklus hidup pesan pengguna, peran setiap layer agen, dan mekanisme eksekusi transaksi terdekopel.</p>
            ...
            <div class="stages-grid">
                <div class="stage-card">
                    <div class="stage-header">
                        ...
                        <div>
                            <h4>Tahap 1: Ingestion & Analysis Pipeline</h4>
                            <span class="stage-sub">Front-Door, Context Memory & Generic Extraction</span>
                        </div>
                    </div>
                    <ul class="detail-list">
                        <li><strong>Conversation Manager</strong>: Menyaring perintah cepat sistem seperti <code>reset</code>, <code>cancel</code>, <code>help</code> tanpa melalui LLM.</li>
                        <li><strong>Memory Service</strong>: Mengupdate dan mengunci preferensi memori pengguna lintas turn.</li>
                        <li><strong>NLU Service (Generic Entity Extractor)</strong>: Mengekstraksi entitas domain (<code>product</code>, <code>size_ml</code>, <code>quantity</code>, <code>payment_method</code>, <code>period</code>) secara generik dari teks pengguna.</li>
                        <li><strong>Event Mapper & Planner Service</strong>: Menerjemahkan intent menjadi event FSM dan menghasilkan <code>ExecutionPlan</code> bertipe yang berisi daftar <code>Action(Enum)</code>.</li>
                    </ul>
                </div>
                ...
                <div class="stage-card">
                    <div class="stage-header">
                        ...
                        <div>
                            <h4>Tahap 2: FSM Routing & Execution Engine</h4>
                            <span class="stage-sub">Pure FSM Router & Agent Registry Dispatching</span>
                        </div>
                    </div>
                    <ul class="detail-list">
                        <li><strong>Workflow Manager (Pure FSM)</strong>: Mengelola transisi state mesin status (FSM) tanpa memiliki dependensi langsung ke nama agen.</li>
                        <li><strong>Execution Engine</strong>: Mengatur eksekusi stage agen (paralel/sekuensial), isolasi kegagalan, dan retry policy.</li>
                        <li><strong>Action Router & Agent Registry</strong>: Memetakan <code>Action(Enum)</code> ke implementasi <code>BaseSpecialistAgent</code> yang sesuai menggunakan pola <em>Dependency Injection</em>.</li>
                    </ul>
                </div>
                ...
                <div class="stage-card">
                    <div class="stage-header">
                        ...
                        <div>
                            <h4>Tahap 3: Specialist Agents & Persistence Layer</h4>
                            <span class="stage-sub">Read/Write Decoupled Services & Repositories</span>
                        </div>
                    </div>
                    <ul class="detail-list">
                        <li><strong>PricingAgent (Action.CHECK_PRICE)</strong>: <em>Business Enrichment</em> murni yang menghitung harga satuan dan subtotal.</li>
                        <li><strong>InventoryAgent (Action.CHECK_STOCK)</strong>: <em>Read-Only Information Provider</em> yang mengecek ketersediaan stok riil gudang tanpa mengubah database.</li>
                        <li><strong>OrderAgent (Action.CREATE_ORDER)</strong>: <em>Atomic Transaction Orchestrator</em> yang mengeksekusi blok transaksi <code>BEGIN ... COMMIT / ROLLBACK</code> SQLite untuk memotong stok dan mencatat <code>sales_history</code>.</li>
                        <li><strong>Repositories Layer</strong>: Mengisolasi penuh kueri SQL (<code>CatalogRepository</code>, <code>InventoryRepository</code>, <code>OrderRepository</code>) dari logika bisnis agen.</li>
                    </ul>
                </div>
                ...
                <div class="stage-card">
                    <div class="stage-header">
                        ...
                        <div>
                            <h4>Tahap 4: Action-Indexed Response Synthesis</h4>
                            <span class="stage-sub">Result Aggregation & Coordinator AI</span>
                        </div>
                    </div>
                    <ul class="detail-list">
                        <li><strong>AgentResult Aggregator</strong>: Utilitas yang menyatukan seluruh objek <code>AgentResult</code> yang terindeks berdasarkan <code>Action(Enum)</code>.</li>
                        <li><strong>Coordinator AI</strong>: Menyintesiskan teks respons percakapan yang ramah pengguna, menyusun Ringkasan Pesanan terstruktur, pilihan pembayaran, hingga Bukti Struk Transaksi resmi (<code>INV-YYYYMMDD-XXXX</code>).</li>
                    </ul>
                </div>
            </div>
        </div>
